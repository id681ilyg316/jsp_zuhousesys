    ...
    <div class="layui-row layui-col-space15">
        <div class="layui-col-md12">
            <div class="layui-housed">
                ...
                <div class="layui-housed-body ">
                    <form class="layui-form layui-col-space5">
                        ...
                        <div class="layui-inline layui-show-xs-block">
                            <input type="text" name="cname" placeholder="请输入民宿屋位置" id="cname" autocomplete="off"
                                   value="${mp.cname[0]}"
                                   class="layui-input">
                        </div>
                        ...
                        <div class="layui-input-inline layui-show-xs-block">
                            <select name="status" id="status">
                                ...
                                <c:if test="${empty mp.status[0]}">
                                    <option value=""  selected>民宿屋状态</option>
                                    <option value="1" >上架</option>
                                    <option value="-1">下架</option>
                                </c:if>
                        ...
                        <div class="layui-inline layui-show-xs-block">
                            <button class="layui-btn" lay-submit="PageHouseServlet" lay-filter="sreach"><i
                ...
                <div class="layui-housed-body layui-table-body layui-table-main">
                    <table class="layui-table layui-form">
                        ...
                        <c:forEach var="house" items="${pu.list}" varStatus="s">
                            <tr>
                                <td>
                                    <input type="checkbox" name="id" value="${house.cid}" lay-skin="primary">
                                </td>
                                <td>${s.index + 1}</td>
                                <td>${house.cname}</td>
                                <td>${house.cinfo}</td>
                                ...
                                <td class="td-manage">
                                    <a onclick="member_stop(this,'${house.cid}','${house.status}')" href="javascript:;"
                                       title="上架">
                    ...
                    <div class="page">
                        <div>
                            <c:if test="${pu.currentPage != 1}">
                                <a class="prev"
                                   href="PageHouseServlet?currentPage=${pu.currentPage-1}&pageSize=5&cname=${mp.cname[0]}&crent=${mp.crent[0]}&status=${mp.status[0]}">&lt;&lt;</a>
                            </c:if>
                            ...
                            <c:forEach end="${pu.totalPage}" var="cpage" begin="1">
                                ...
                                <c:if test="${pu.currentPage != cpage}">
                                    <a class="num"
                                       href="PageHouseServlet?currentPage=${cpage}&pageSize=5&cname=${mp.cname[0]}&crent=${mp.crent[0]}&status=${mp.status[0]}">${cpage}</a>
                                </c:if>
                            </c:forEach>
...
<script>
    ...
    function delAll(argument) {
        ...
        layer.confirm('确认要下架吗？', function (index) {
                location.href = "${pageContext.request.contextPath}/HouseChangeSelectServlet?status=-1&cids=" + cids;
            }
        );
    }
